...
//#ifdef IGNORE_CASE
                ...
                case 'i':
                        ...
                        getdata(21, 0, "§ó§ïID¤j¤p¼g : ", nfakeid, sizeof(nfakeid),
...
int x_username()
{
	...
	outs("\n\
©êºp, ¥Ñ©ó±zªº¼ÊºÙ§t¦³±m¦â±±¨î½X, ¥B¥¼¥[¤WÁÙ­ì½X, ¬°¤F¨Ï±o¥X²{¦b\n\
«H¥ó¡B§G§i¡B¨Ï¥ÎªÌ¦Cªí¤¤¤§±m¦â±±¨î½X, ¤£­P¤Þ°_µe­±¶Ã±¼ªº°ÝÃD,\n\
©Ò¥H»Ý­n±z§@¥H¤U¼ÊºÙ­×¥¿ªº¨BÆJ: \n");
	if (!buf)
	{
		outs("\n¨t²Îµo¥Í¿ù»~! ½Ð³qª¾¨t²ÎºÞ²z­û!");
		return C_FULL;
	}
	
	strcpy(buf, curuser.username);
	move(10, 0);
	prints("\n±z­ì¥»ªº¼ÊºÙ¬°: %s", curuser.username);
	s = buf;
	t = buf;
	while (*s)
	{
