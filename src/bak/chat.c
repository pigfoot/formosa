...
t_chat()
{
	...
	/* initial ignore list */
	if (!(iglist = malloc_array(MAX_IGNORE_USER)))
		return C_FULL;
	...
	while (1)
	{
		...
		if (ch == I_OTHERDATA)
		{
			...
			if (rcvbuf[0] == '/')
			{
				...
				{
					*p = '\0';
					if (cmp_array(iglist, rcvbuf + 1, strcmp))
						continue;
					...
					if ((p = strchr(nick, '\t')) != NULL)
					{
						*p = '\0';
						...
						if (cmp_array(iglist, nick, strcmp))
							continue;
	...
	else if (!strcmp(cmd, "ignore") || !strcmp(cmd, "i"))
	{
		if (*para)
		{
			if (strlen(para) >= IDLEN)
				para[IDLEN - 1] = '\0';
			if (!cmp_array(iglist, para, strcmp))
			{
				sprintf(genbuf, _msg_chat_37, para);
				printchatline(genbuf);
				add_array(iglist, para, malloc_str);
			}
		}
		return;
	}
