...
alloc_line()
{
	...
	if (total_num_of_line > 2048)	/* lthuang: debug */
	{
		indigestion(3);
		abort_bbs(0);
	}
...
char *bname;
{
	...
	getdata(0, 0, genbuf, abort, sizeof(abort), ECHONOSP | LOWCASE, NULL);
	if (abort[0] == 'a')
	{
		vedit_exit();
		outs("\nFile NOT saved\n");
...
char *bname;
{
	...
	if (!isfile(filename))
		unlink(bakfile);
	if (saveheader)
		do_article_sig(curuser.userid, filename);
