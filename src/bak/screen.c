...
clrtobot()
{
	...
	{
		big_picture[(i + roll) % t_lines].mode = 0;
		big_picture[(i + roll) % t_lines].len = 0;
		if (big_picture[(i + roll) % t_lines].oldlen > 0)
			big_picture[(i + roll) % t_lines].oldlen = 255;
		big_picture[(i + roll) % t_lines].width = 0;
	}
}
...
scroll()
{
	...
	scrollcnt++;
	roll = (roll + 1) % t_lines;
/* subtract one from roll mod t_lines */
	move(t_lines - 1, 0);
	clrtoeol();
}
...
#if 0
struct screenline *save_big_picture;
#endif
...
save_screen()
{
	getyx(&save_y, &save_x);
#if 0
	save_big_picture = (struct screenline *) calloc(t_lines,
						   sizeof(struct screenline));
#endif
	memcpy(save_big_picture, &(big_picture[(t_lines - 2 + roll) % t_lines]),
	       sizeof(struct screenline));
	memcpy(save_big_picture + 1, &(big_picture[(t_lines - 1 + roll) % t_lines]),
	       sizeof(struct screenline));
#if 0
	memcpy(save_big_picture, big_picture,
	       t_lines * sizeof(struct screenline));
#endif
}
...
#if 0
	free(big_picture);
	big_picture = save_big_picture;
	save_big_picture = (struct screenline *) NULL;
#endif
	memcpy(&(big_picture[(t_lines - 2 + roll) % t_lines]), save_big_picture,
	       sizeof(struct screenline));
	memcpy(&(big_picture[(t_lines - 1 + roll) % t_lines]), save_big_picture + 1,
	       sizeof(struct screenline));
	slp = &(big_picture[(t_lines - 2 + roll) % t_lines]);
	slp->oldlen = 255;
	slp->mode |= MODIFIED;
	slp->smod = 0;
	slp->scmod = 0;
	slp = &(big_picture[(t_lines - 1 + roll) % t_lines]);
	slp->oldlen = 255;
	slp->mode |= MODIFIED;
	slp->smod = 0;
	move(save_y, save_x);
