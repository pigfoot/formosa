...
int 
select_file_cache_slot(char *file)
{
	int slot, selected = 0;
#ifdef USE_WEIGHT
...
#ifdef USE_WEIGHT
		weight = test_weight(request_rec->atime, file_shm[slot].atime, file_shm[slot].hit);
...
int 
select_html_cache_slot(char *file)
{
	int slot, selected = 0;
#ifdef USE_WEIGHT
	int weight = 0, min_weight = 0;
...
#ifdef USE_WEIGHT
		weight = test_weight(request_rec->atime, html_shm[slot].atime, html_shm[slot].hit);
		if (min_weight > weight)
		{
			min_weight = weight;
			selected = slot;
...
do_cache_file(char *file)
{
	...
	file_shm[slot].ctime = request_rec->atime;
