...
int ParseCommand P_((char *inbuf));
...
/* uri.c */
void GetURIToken P_((char *boardname, char *post, char *skin, const char *curi));
int ParseURI P_((const char *curi, REQUEST_REC *r, BOARDHEADER *board, POST_FILE *pf));
/* user.c */
...
int CheckAccessList P_((char *boardname, char *userid));
int ModifyAccessList P_((char *pbuf, BOARDHEADER *board, POST_FILE *pf));
