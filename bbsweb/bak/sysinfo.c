...
#ifdef PRE_FORK
		char atime[30];
		char *ss[] = {"READY", "BUSY", "SIGTERM", "SIGSEGV", "SIGPIPE", "ERROR", "WAIT", "ACCEPT", "ENOSPC"};
#endif
	...
	else if(!strcasecmp(tag, "CLIENTINFO"))
	{
		...
		for(i=0; i<MAX_NUM_CLIENT; i++)
		{
			fprintf(fp_out, "[%02d] from=%s, proxy=%s, uri=%s<br>UA=%s<br>\n",
				i+1,
				server->client_record[i].fromhost,
				server->client_record[i].proxyhost,
				server->client_record[i].URI,
				server->client_record[i].user_agent
				);
		}
	}
#endif
