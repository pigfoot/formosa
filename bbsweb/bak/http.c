...
void ShowHttpHeader(REQUEST_REC *r, SKIN_FILE *sf, POST_FILE *pf)
{
	...
	/* ===== Primary Respond ===== */
	fprintf(fp_out, "%s\r\n", http_header[r->HttpRespondType].status_line);		/* Status-Line */
...
/*
 	...
 	so ...........
*/
	if(http_header[r->HttpRespondType].Pragma && sf->expire)
		fprintf(fp_out, "Pragma: no-cache\r\n");

	if(http_header[r->HttpRespondType].Content_Type)
		fprintf(fp_out, "Content-Type: %s\r\n", mime_type[sf->mime_type].type);

	if(http_header[r->HttpRespondType].Content_Length && sf->mime_type > 1)
		fprintf(fp_out, "Content-Length: %d\r\n", sf->size);

	if(http_header[r->HttpRespondType].Location)
		fprintf(fp_out, "Location: http://%s%s\r\n", r->host, sf->filename);

	if(http_header[r->HttpRespondType].WWW_Authenticate)
 ...
 *	return: none
 *******************************************************************/
int ParseHttpHeader(REQUEST_REC *r, SERVER_REC *s)
{
	char *buffer;
	int count=0;
	...
	if(!isTORNADO)
			...
			||  !strncasecmp(buffer, "X-SeederNet-For:", 16))
		{
			...
			xstrncpy(r->x_forward_for, buffer+17, STRLEN*2);
			buffer+=17;
			strtok(buffer, ",\r\n");
		...
		else if(!strncasecmp(buffer, "Referer:", 8))
		{
			...
			*/
			...
			&& strncasecmp(buffer+16, "140.117.99.", 11))
				weblog_line(buffer+16, s->referer_log, r->fromhost, r->atime);
#else
			if(strncasecmp(buffer+16, s->host_name, strlen(s->host_name))
			&& strncasecmp(buffer+16, s->host_ip, strlen(s->host_ip)))
				weblog_line(buffer+16, s->referer_log, r->fromhost, r->atime);
#endif
#endif
		...
		*/
		...
		else	/* log other unknow header */
		{
			weblog(buffer, WEB_OTHERHEADER_LOG, r->fromhost);
		}
